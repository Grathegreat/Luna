         ...
         fs.readdir(folderPath, (err, files) => {
           ...
           if (latestFile) {
             ...
             if (checker.bestMatch.rating >= 0.5) {
              ...
              api.shareContact(`==[ ${gio} ]==\n🌌 ${global.config.BOTNAME} chào bạn!\n✅ Lệnh bận cần là: ${checker.bestMatch.target}?\n─────────────────\n🎃 Lệnh vừa load: ${newFile}\n🎶 Uptime: ${hours}:${minutes}:${seconds}`, uid, event.threadID, async (err, info) => {
                 await new Promise(resolve => setTimeout(resolve, 15 * 1000));
                 return api.unsendMessage(info.messageID);
               }, event.messageID);
             }
